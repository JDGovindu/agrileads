...
with st.sidebar:
    ...
    st.markdown("#### 0001F511 GOOGLE API KEYS")
    ST.CAPTION("REQUIRED TO SEARCH. FREE 2014 100 SEARCHES/DAY.")
    API_KEY = ST.TEXT_INPUT("GOOGLE API KEY", TYPE="PASSWORD", PLACEHOLDER="AIZA...", HELP="GET FROM CONSOLE.CLOUD.GOOGLE.COM")
    CX = ST.TEXT_INPUT("SEARCH ENGINE ID (CX)", PLACEHOLDER="A1B2C3...", HELP="GET FROM PROGRAMMABLESEARCHENGINE.GOOGLE.COM")
    KEYS_OK = BOOL(API_KEY AND CX)
    IF NOT KEYS_OK:
        ST.CAPTION("26A0FE0F ADD API KEYS ABOVE TO ENABLE SCRAPING")

    scrape_clicked = st.button("▶  Scrape Leads Now", type="primary", use_container_width=True, disabled=not keys_ok)
...
if scrape_clicked:
    if not keywords:
        ...
        try:
            with st.spinner("🔍 Searching via Google Custom Search API..."):
            ...
            leads = st.session_state.leads
            st.success(f"✅ Found {len(new_leads)} results — {added} new leads added!")
